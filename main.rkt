...
(define (assert test message . args)
  (unless test
    (apply error (~a message " ... aborting") args)
    (exit -1)))
...
(define (update)
  ...
  (when (build-env?)
    ...
    (define nix-env-installed-versions-dir (:/ (nix-env-config)
					       "nix-env-installed-version"))
    (when (not (directory-exists? nix-env-installed-versions-dir))
      (make-directory nix-env-installed-versions-dir)) 
...
(module+ main
  ...
  ;; validation
  (assert (directory-exists? (nixpkgs)) "nixpkgs directory does not exist: ~a"
	  (nixpkgs))
  (define default-channels-dir
    (build-path (getenv "HOME") ".nix-defexpr/channels_root"))
  (when (and (directory-exists? default-channels-dir)
	     (not (set-empty?
		   (list-nix-channels default-channels-dir))))
    (warn "There seems to be a channel configured by `nix-channel'. This setup might interfere with nixos-update in unexpected ways. It is recommended to remove all channels using `sudo -i nix-channel --remove <channel name>")
    )
